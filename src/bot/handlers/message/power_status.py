...
async def handle_power_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user is None:
        logger.bind(username="system").warning("Received power status request but effective_user is None")
        return
    ...
    async with session_factory() as session:
        ...
        if latest_status is None:
            logger.bind(username=username).warning("No status records of Status found in DB")
